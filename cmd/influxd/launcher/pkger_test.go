	...
	t.Run("managing pkg lifecycle with stacks", func(t *testing.T) {
		...
		t.Run("stack ownership", func(t *testing.T) {
			t.Run("labels", func(t *testing.T) {
				...
				// b/c the resource is owned by a different stack, this should only update
				// the resources stack refs is all.
				...
				// b/c the resource is owned by a the provided stack ID, this should update
				// the resources but not create any new references.
				_, err = svc.Apply(ctx, l.Org.ID, l.User.ID,
					pkger.ApplyWithTemplate(newLabelTemplate("foo", "new desc", "peru")),
				...
				// this application uses a different stack so it does not update the resource
				// except for the stack references
				...
				// validate we are handling reference that get reinstalled
				nukedImpact, err = svc.Apply(ctx, l.Org.ID, l.User.ID,
					pkger.ApplyWithTemplate(newLabelTemplate("foo", "", "")),
				...
				// when removing a stack that references the resoruce, should be removed from the
				// resources stack references
